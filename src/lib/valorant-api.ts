...
import { getAccountId, type ProfileSnapshot, type StoreSnapshot, type StoredAuthTokens, type StoredRiotAccount, type ValorantShard } from '@/lib/account';
...
import {
  buildRiotStoreAlertOffers,
  buildRiotStoreSnapshot,
  type RiotStorefrontResponse,
  type StoreAlertOfferSnapshot,
} from '@/lib/riot-storefront';
...
export type { StoreAlertOfferSnapshot } from '@/lib/riot-storefront';
...
export async function fetchStoreAlertOffersWithExistingTokens(
  ...
  return buildRiotStoreAlertOffers(storefront);
}
...
async function fetchStoreSnapshotWithTokens(
  ...
  const storefront = await fetchStorefrontWithTokens(account, tokens);
  return buildRiotStoreSnapshot(storefront);
}

async function fetchStorefrontWithTokens(account: StoredRiotAccount, tokens: StoredAuthTokens) {
  const headers = await getAuthorizedHeaders(tokens);
  return riotFetch<RiotStorefrontResponse>(`https://pd.${account.shard}.a.pvp.net/store/v3/storefront/${account.puuid}`, {
    method: 'POST',
    headers,
    body: JSON.stringify({}),
  });
