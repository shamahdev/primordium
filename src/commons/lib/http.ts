import { ACCOUNT_FALLBACK_CLIENT_VERSION, ACCOUNT_RIOT_CLIENT_PLATFORM } from '@/modules/account/account-constants';
...
export function isRiotHttpAuthFailure(error: unknown) {
  return error instanceof RiotHttpError && (error.status === 401 || error.status === 403);
}
...
export async function buildRiotAuthorizedHeaders(tokens: Pick<AccountTokens, 'accessToken' | 'entitlementsToken'>) {
  return {
...
export async function riotFetch<T>(url: string, init: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    throw new RiotHttpError(`Riot API request failed: ${response.status} ${response.statusText}`, response.status);
  }
