...
export function useCompanionViewModel() {
  ...
  const fetchRank = React.useCallback(async () => {
    const account = useAccountStore.getState().accounts.find((item) => item.id === activeAccountId);
    ...
    try {
      ...
      setRankError(error instanceof Error ? error.message : 'Could not load competitive rank.');
    } finally {
  ...
  const fetchMatches = React.useCallback(async () => {
    ...
    try {
      ...
      setMatchesError(error instanceof Error ? error.message : 'Could not load recent matches.');
    } finally {
