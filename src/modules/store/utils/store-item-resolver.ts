...
export const StoreItemResolver = {
	async resolveStoreItem(params: ResolveStoreItemParams): Promise<StoreItem> {
		const resolvedItemType =
			params.itemType ?? getStoreItemTypeFromReward(params.reward);
		const asset = params.reward
			? await CatalogService.getStoreItemAsset(params.reward.ItemID, {
					refreshOnMiss: true,
				})
			: undefined;
		...
		const favoriteTargetId = params.reward
			? await StoreItemResolver.resolveFavoriteTargetId({
					...
					title,
					imageUrl:
						typeof asset?.imageUrl === "string" ? asset.imageUrl : undefined,
					rarity: asset?.rarity,
				})
			: undefined;
	...
	async resolveItemDetailIdentity(
		...
		const heroSource =
			asset?.animationUrl ?? asset?.largeImageUrl ?? asset?.imageUrl;
	...
	async resolveFavoriteTarget(
		...
		const rawId =
			typeof source === "string" ? source : getRawFavoriteTargetId(source);
		...
		return typeof source === "string"
			? null
			: getFallbackFavoriteTarget(source);
	},

	async resolveFavoriteTargetId(
		source: string | FavoriteTargetSource,
	): Promise<string> {
	...
	async resolveFavoriteOfferMatches(
		...
		for (const offer of offers) {
			...
			const favoriteTargetId = await StoreItemResolver.resolveFavoriteTargetId(
				offer.itemAssetId,
			);
