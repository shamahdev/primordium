...
let preparedCatalogItemsRefreshPromise: Promise<CatalogListItem[]> | null =
	null;
let competitiveTiersPromise: Promise<Map<number, CompetitiveTier>> | null =
	null;
...
async function loadItemAssets() {
	const [skins, buddies, playerCards, sprays, flexes, playerTitles] =
		await Promise.all([
	...
	for (const skin of skins) {
		...
		const rarity = skin.contentTierUuid
			? CONTENT_TIER_TO_RARITY[skin.contentTierUuid]
			: undefined;
		const skinAsset = { title: skin.displayName, imageUrl, rarity };
		const skinCatalogItem: CosmeticCatalogItem = {
			id: skin.uuid,
			itemType: "skin",
			...skinAsset,
		};
		...
		for (const level of skin.levels) {
			items.set(level.uuid, {
				title: skin.displayName,
				imageUrl: level.displayIcon ?? imageUrl,
				rarity,
			});
	...
	for (const buddy of buddies) {
		const imageUrl =
			buddy.displayIcon ?? buddy.levels[0]?.displayIcon ?? undefined;
		const buddyAsset = { title: buddy.displayName, imageUrl };
		const buddyCatalogItem: CosmeticCatalogItem = {
			id: buddy.uuid,
			itemType: "buddy",
			...buddyAsset,
		};
	...
	for (const playerCard of playerCards) {
		...
		const cardCatalogItem: CosmeticCatalogItem = {
			id: playerCard.uuid,
			itemType: "card",
			...cardAsset,
		};
	...
	for (const spray of sprays) {
		...
		const sprayCatalogItem: CosmeticCatalogItem = {
			id: spray.uuid,
			itemType: "spray",
			...sprayAsset,
		};
	...
	for (const flex of flexes) {
		const flexAsset = {
			...
			imageUrl:
				flex.verticalPromoImage ??
				flex.displayIcon2 ??
				flex.displayIcon ??
				undefined,
		};
		items.set(flex.uuid, flexAsset);
		const flexCatalogItem: CosmeticCatalogItem = {
			id: flex.uuid,
			itemType: "flex",
			...flexAsset,
		};
	...
	for (const playerTitle of playerTitles) {
		const title =
			playerTitle.displayName ?? playerTitle.titleText ?? "Player Title";
		const titleAsset = { title };
		items.set(playerTitle.uuid, titleAsset);
		const titleCatalogItem: CosmeticCatalogItem = {
			id: playerTitle.uuid,
			itemType: "title",
			...titleAsset,
		};
...
async function loadCompetitiveTiers() {
	const episodes = await fetchCatalog<{ tiers: CompetitiveTier[] }>(
		"competitivetiers",
	);
...
async function loadAgents() {
	const agents = await fetchCatalog<
		AgentAsset & { role?: { displayName?: string | null } | null }
	>("agents");
	...
	async getPreparedCatalogItems({ refresh = false } = {}) {
		if (refresh) {
			if (!preparedCatalogItemsRefreshPromise) {
				...
				preparedCatalogItemsRefreshPromise =
					preparedCatalogItemsPromise.finally(() => {
