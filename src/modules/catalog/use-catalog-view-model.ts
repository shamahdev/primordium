...
export function useCatalogViewModel() {
	...
	const [favoritesOnly, setFavoritesOnly] = React.useState(
		catalogMemoryFavoritesOnly,
	);
	const [typeFilter, setTypeFilter] = React.useState<CatalogItemType[]>(
		catalogMemoryTypeFilter,
	);
	const [rarityFilter, setRarityFilter] = React.useState<CatalogRarity[]>(
		catalogMemoryRarityFilter,
	);
	const [ownedStatus, setOwnedStatus] = React.useState<OwnedStatus>(
		catalogMemoryOwnedStatus,
	);
	...
	const loadCatalog = React.useCallback(
		async (refresh = false, showLoading = true) => {
			if (showLoading) {
				setLoading(true);
				setError(null);
			}
			...
			try {
				...
				nextError =
					loadError instanceof Error
						? loadError.message
						: "Could not load cosmetic catalog.";
			}
	...
	React.useEffect(() => {
		...
		void (async () => {
			...
			try {
				nextItems = await CatalogService.getPreparedCatalogItems({
					refresh: false,
				});
			} catch (loadError) {
				nextError =
	...
	const favoriteItems = React.useMemo(
		() =>
			sortFavoriteItems(
				Object.values(favoritesById).map((favorite) => ({
	...
	const handleToggleRarity = React.useCallback((rarity: CatalogRarity) => {
		setRarityFilter((prev) => {
			const next = prev.includes(rarity)
