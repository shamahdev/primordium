...
export const OwnedItemsService = {
  async fetchOwnedItemIds(account: Account): Promise<Set<string>> {
    return StoredRiotSession.request(account, (tokens) => fetchOwnedItemIdsWithTokens(account, tokens));
  },
};

async function fetchOwnedItemIdsWithTokens(account: Account, tokens: AccountTokens): Promise<Set<string>> {
