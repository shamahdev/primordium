...
export function useAccountSwitchViewModel() {
	...
	const chooseAccount = async (accountId: string) => {
		...
		try {
			...
			log.nav.debug("switch-account choose: dismissAll + replace returnTo", {
				accountId,
				returnTo,
				target,
			});
	...
	const cancel = () => {
		...
		log.nav.debug("switch-account cancel: replace fallback", {
			returnTo,
			fallback,
		});
	...
	const handleRemoveAccount = async (accountId: string) => {
		...
		if (nextActiveId) {
			router.replace(
				getSwitchAccountHref({ reason: "afterRemoval", returnTo }),
			);
		} else {
