...
import type { Account, AccountTokens, AccountCookie } from '@/modules/account/account-type';
import { getAccountLabel, isAccountShard } from '@/modules/account/account-type';
...
export function useAccountLoginViewModel() {
  const params = useLocalSearchParams<{ mode?: AccountLoginMode; shard?: string; accountId?: string; returnTo?: string }>();
  ...
  const shard = mode === 'reauth' ? reauthAccount?.shard : isAccountShard(params.shard) ? params.shard : undefined;
  ...
  const saveAndContinue = async (result: LoginAuthenticatedResult) => {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : 'Failed to save login.');
    } finally {
      setIsSaving(false);
    }
  };
