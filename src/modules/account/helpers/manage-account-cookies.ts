...
import {
	ACCOUNT_RIOT_AUTH_COOKIE_URLS,
	ACCOUNT_RIOT_AUTH_DOMAINS,
	...
	for (const { hostname, jars } of results) {
		for (const jar of jars) {
			for (const [fallbackName, cookie] of Object.entries(jar)) {
				...
				cookies.set(
					`${normalized.domain}|${normalized.path ?? "/"}|${normalized.name}`,
					normalized,
				);
			}
		}
...
function isRiotAuthDomain(domain: string) {
	...
	return ACCOUNT_RIOT_AUTH_DOMAINS.some(
		(allowed) =>
			normalized === allowed ||
			normalized.endsWith(`.${allowed.replace(/^\./, "")}`),
	);
}
...
async function getCookieJars(url: string) {
	if (Platform.OS === "ios") {
		const results = await Promise.allSettled([
			CookieManager.get(url, false),
			CookieManager.get(url, true),
		]);
		return results.flatMap((result) =>
			result.status === "fulfilled"
				? [result.value as Record<string, NativeCookie>]
				: [],
		);
	}
...
async function setCookie(url: string, cookie: AccountCookie) {
	...
	if (Platform.OS === "ios") {
		await Promise.allSettled([
			CookieManager.set(url, nativeCookie, false),
			CookieManager.set(url, nativeCookie, true),
		]);
		return;
	}
