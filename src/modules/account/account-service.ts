...
async function fetchProfileWithTokens(
	account: Account,
	tokens: AccountTokens,
): Promise<AccountProfile> {
	const headers = await buildRiotAuthorizedHeaders(tokens);
	const [xp, wallet] = await Promise.all([
		riotFetch<AccountXPResponse>(
			`https://pd.${account.shard}.a.pvp.net/account-xp/v1/players/${account.puuid}`,
			{
				headers,
			},
		),
		riotFetch<WalletResponse>(
			`https://pd.${account.shard}.a.pvp.net/store/v1/wallet/${account.puuid}`,
			{
				headers,
			},
		),
...
export const AccountService = {
	...
	fetchProfile(account: Account): Promise<AccountProfile> {
		return StoredRiotSession.request(account, (tokens) =>
			fetchProfileWithTokens(account, tokens),
		);
	},
