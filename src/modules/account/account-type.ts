...
export type AccountRecoveryKind = 'interactiveLoginRequired' | 'temporaryAuthUnavailable';
...
export function isAccountRecoveryError(error: unknown): error is AccountRecoveryError {
...
export type AccountReturnToRoute = '/home' | '/profile' | `/onboarding?shard=${AccountShard}`;

export function isAccountShard(value: unknown): value is AccountShard {
  return typeof value === 'string' && ACCOUNT_SHARDS.some((shard) => shard.id === value);
}
...
export function getAccountLabel(account: Pick<Account, 'displayName' | 'gameName' | 'tagLine' | 'puuid'>) {
