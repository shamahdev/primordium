...
export function useAccountOnboardingViewModel() {
	...
	const initialShard = ACCOUNT_SHARDS.some((shard) => shard.id === params.shard)
		...
		: "eu";
	const [selectedShard, setSelectedShard] =
		React.useState<AccountShard>(initialShard);

	const startLogin = () => {
		router.push(
			getLoginHref({ mode: "add", shard: selectedShard, returnTo: "/home" }),
		);
	};
	...
	const switchAccount = () => {
		router.push(
			getSwitchAccountHref({
				reason: "choose",
				returnTo: buildOnboardingReturnTo(selectedShard),
			}),
		);
	};
