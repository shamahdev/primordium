...
import {
	ACCOUNT_RIOT_INTERACTIVE_LOGIN_HOST,
	ACCOUNT_RIOT_INTERACTIVE_LOGIN_PATH,
...
export function AccountSessionRefreshWebView() {
	...
	const finishJob = (
		currentJob: SessionRefreshJob,
		result: AccountSessionRefreshResult,
	) => {
	...
	React.useEffect(() => {
		return registerWebViewRefreshAdapter(
			({ sourceUri }) =>
	...
	const handleUrl = (url?: string) => {
		...
		if (
			url.includes(ACCOUNT_RIOT_INTERACTIVE_LOGIN_HOST) &&
			url.includes(ACCOUNT_RIOT_INTERACTIVE_LOGIN_PATH)
		) {
			...
			onError={() => {
				...
				if (currentJob) {
					finishJob(currentJob, {
						kind: "networkUnavailable",
						message: "Could not reach Riot login.",
					});
				}
			}}
