...
type SessionRefreshWebViewAdapter = (request: SessionRefreshRequest) => Promise<AccountSessionRefreshResult>;
...
export function registerWebViewRefreshAdapter(nextAdapter: SessionRefreshWebViewAdapter) {
  adapter = nextAdapter;
...
export async function waitForWebViewRefreshAdapter(): Promise<SessionRefreshWebViewAdapter> {
  ...
  const nextAdapter = await new Promise<SessionRefreshWebViewAdapter | null>((resolve) => {
