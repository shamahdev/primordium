...
async function refreshTokens(account: Account, force: boolean): Promise<AccountTokens> {
  ...
  try {
    ...
    const authenticated = await authenticateLogin(result.accessToken, account.shard);
    ...
    await useAccountStore
      .getState()
      .saveAuthenticatedAccount(authenticated.account, authenticated.tokens, result.cookies);
...
async function withAuthorizedTokens<T>(
  ...
  try {
    ...
    log.api.warn('withAuthorizedTokens: first attempt auth failure, refreshing', {
      accountId: account.id,
    });
  }
  ...
  try {
    ...
    if (isRiotHttpAuthFailure(error)) {
      log.api.error('withAuthorizedTokens: second attempt auth failure, rejecting', {
        accountId: account.id,
      });
      await rejectAuth(account.id, 'cookieReauthFailed');
    }
...
export const StoredRiotSession = {
  ...
  request<T>(account: Account, callback: (tokens: AccountTokens) => Promise<T>): Promise<T> {
